...
class ThreadController extends Controller
{
    ...
    {
        return $this->middleware([Authenticate::class, EnsureEmailIsVerified::class])->except(['index', 'show', 'search', 'noLike', 'noReply']);
    }
    ...
    public function show(Category $category, Thread $thread)
    {
        ...
        views($thread)->cooldown($expireAt)->record();
    ...
    {
        return view('pages.threads.search', [
            'threads' => Thread::latest()->filter(request(['search']))->paginate(10)
        ]);
    }
    ...
    public function noReply()
    {
        ...
        foreach ($threads as $thread) {
            if (count($thread->replies()) === 0) {
                array_push($noReplyThreads, $thread);
            }
        }
        return view('pages.threads.no-replies', [
            'threads' => $noReplyThreads,
        ]);
    }
    ...
    public function noLike()
    {
        ...
        foreach ($threads as $thread) {
            if (count($thread->likes()) === 0) {
                array_push($noLikeThreads, $thread);
            }
        }
        return view('pages.threads.no-likes', [
            'threads' => $noLikeThreads,
        ]);
    }
}
